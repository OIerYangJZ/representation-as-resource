...
\pdfoutput=1
% Supplemental material, Quantum (quantum-journal.org) format.  The PRX
% Quantum (revtex4-2) version is frozen under prxquantum_format_backup/.
...
% quantumarticle pulls in revtex's ltxgrid/ltxutil; its legacy array patch
% is incompatible with array 2.6n (TeX Live 2026).
\makeatletter
...
\title{Supplemental Material for ``Representation-Dependent
Recoverability in Quantum Compilation''}
\author{Jinze Yang}
...
\captionof{figure}{Pareto and sensitivity views generated from the complete W7 parquet
...
\begin{center}
\begin{minipage}{\textwidth}
\captionof{table}{Matched structured-family result retained without cherry-picking.}
...
\captionof{table}{All six natural-family gate-count main effects.  Off-minus-on is
...
\midrule
\input{generated/ablation_gate_effects}
\end{tabular}
\end{minipage}
\end{center}
...
The line-topology preflight caught an invalid final-layout permutation in a
candidate router, whose dense certificate failed before the frozen campaign.
We replaced it with an explicit adjacent CX-SWAP construction restoring
identity layout and reran the entire campaign.  The failed preflight is
retained because correctness takes precedence over favorable metrics.

\section{Legacy and stress-result registry}
\label{supp:legacy}

The active submission body is the full-length
\path{PaperDraft/main.tex}.  The unintegrated 38-page source is frozen as
\path{PaperDraft/legacy_main_v7_2.tex}, and the superseded nine-page
reconstruction is frozen as \path{PaperDraft/short_main_w10.tex}.  Neither frozen
...
The committed repository is sufficient for this frozen-data reconstruction.
The approximately 700-MB raw W3--W8 run tree is versioned separately in the
private \path{OIerYangJZ/paper1-data} archive at the revision recorded in
\path{data/manifest.yaml}; the audit verifies its registered hashes when that
optional archive is materialized.  The one-command rebuild does not claim to
rerun the complete measurement campaigns.
