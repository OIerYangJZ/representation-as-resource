...
(&\texttt{experiment},\texttt{instance},\texttt{representation},
  \texttt{method},\texttt{seed},\\
 &\texttt{backend\_hash},\texttt{tool\_version},\texttt{config\_hash}).
\end{split}
\end{equation*}
The backend and configuration hashes are SHA256 digests of canonical JSON
(sorted keys and no insignificant whitespace).  A row collision on all eight
...
& legacy baseline repo; semantic candidate controls absent
...
The theorem-model reference compiler is
\path{scripts/trace_cut_budget.py}.  Its event schema records after every input update
\texttt{committed\_output\_bytes}, the five cut fields from
\eqref{eq:info-budget}, pass index, RSS, elapsed nanoseconds, artifact paths,
and digests.  It deletes the explicit live compiler-state object and resumes
from only the committed prefix, five serialized fields, and unread input
suffix.  The frozen \path{research/cut_trace/} bundle was produced by the
earlier serializer and lacks the per-event field files, so its 1,320 events and
24 summaries are historical only and are not accepted by the version-one
verifier.  The checked three-cut codec fixture and its manifests are in the
project-root \path{instrumentation/example_trace/}; a full empirical curve
requires rerunning this revised tracer.
...
\midrule
\input{research/claim_evidence/claim_evidence_rows.tex} \\
\bottomrule
\end{tabularx}
