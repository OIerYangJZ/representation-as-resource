% Submission manuscript.  The W10 short reconstruction is preserved as
...
Time-dependent and variational schedules split one accumulated phase
across many rounds, gate synthesis replaces an angle by a Clifford+$T$
word in which no individual gate carries the angle at all, and
randomized compiling twirls each rotation into sign-randomized
fragments.
We show that this dispersal has a price, and that the price is payable
in exactly two currencies: bits of compiler state, or magic states.

We formalize a downstream compiler---one positioned after randomization
and lowering, as a cloud transpiler or a control-stack stream optimizer
is---and charge every restart channel in bits: control, store, live
window, parameters, and intermediate representation.  For a commuting
phase layer with $m$ generators compiled to projective accuracy
$\epsilon$, the physically realized stream is an $r$-round modular
accumulation on the Ross--Selinger synthesis grid
$Q_\epsilon=K_\epsilon+1=\Theta(1/\epsilon)$, whose exact packing has
separation $2\sin(\pi/(2Q_\epsilon))$.  We prove that any such compiler
making $p$ forward passes and failing with probability at most $\delta$
obeys
\begin{equation*}
 \overline A_p+(2p-1)S\geq
 (1-\delta)m\log_2K_\epsilon-h_2(\delta),
\end{equation*}
where $\overline A_p$ is committed output and $S$ the serialized
crossing state.
...
The bound is carried by the schedule alone: sign normalization is a
bijection of the residue alphabet, so the standard folded
(compile-time) presentation of randomized compiling inherits it
verbatim, and no claim depends on tracking frames.  Under the
additional disclosed frame-tracked presentation the state $S$ is the
Pauli frame itself, and the bound expresses a direct tension: the
randomization that averages coherent noise is the same randomization
that forces a
re-aggregating compiler to carry $\Omega(m\log(1/\epsilon))$ bits.  A memory-capped
block compiler attains the envelope within a constant factor; direct
semantic aggregation attains the compact-output corner with logarithmic
state.
...
Propagating one fixed total error budget through actual Clifford+$T$
synthesis and surface-code scheduling turns the representational gap
into a fault-tolerant one: the same unitary, compiled from a dispersed
rather than an aggregate representation, costs orders of magnitude more
logical $T$ states and spacetime volume under the disclosed model.

The two currencies are not, however, worth the same.  Once a committed
record is executed rather than buffered, the committed channel pays a
toll the state channel never pays: the entropy $\log_2\binom{m}{k}$ of
the compiler's own decision about which $k$ coordinates to commit
early.  A compiler that defers is handed that information for free by
the stream that follows.  Under a preregistration frozen before the
run, the toll is measured to match the predicted entropy, to be
independent of $m$, and to vanish exactly when the commitment schedule
is public.

The operational conclusion is a design rule with a proof: randomize and
lower late, because the information a compiler discards before
aggregating cannot be recovered for free---it must be bought back in
memory or in magic states, and buying it back early is strictly the
...
A fault-tolerant compiler is not only a translator; it is also, by
design, a destroyer of structure.  Time-dependent schedules and
...
into a Clifford+$T$ word whose individual letters carry none of
it~\cite{dawson2006solovay,kliuchnikov2013fast,selinger2015efficient,ross2016optimal}.
...
is correct, standard, and desirable for its own
reason---and each of them takes a quantity that was once written down in
one place and scatters it across an instruction stream.

This paper asks what it costs to put such a quantity back together, and
shows that the cost is not a matter of engineering
skill~\cite{fellous2023optimizing,watkins2024surfacecode,beverland2022assessing}.  It is an
information-theoretic quantity with two admissible payment channels.  A
...
later in fault-tolerant resources.  There is no third option, and the
exchange rate between the two is fixed by the accuracy $\epsilon$ and
the number of phase terms $m$.
...
committed to output and repaid downstream in magic states.
...
further $\log_2\binom{m}{k}$ when the commitment schedule is private.}
...
$L_\epsilon=\lfloor\pi/(8\arcsin2\epsilon)\rfloor=196$ and hence
$K_\epsilon=197$ distinguishable settings per edge
(\cref{prop:epsilon-packing}), so each edge total is worth
$\log_2197=7.62$ bits and the layer as a whole is worth $252$ bits, or
about $31$ bytes.  A compiler that defers aggregation carries those
$31$ bytes across every cut.  A compiler that commits early does not,
and buys the same information back in the only other currency
available: each coordinate it must afterwards correct costs a fresh
Ross--Selinger synthesis of roughly $3\log_2(1/\epsilon)\approx30$ $T$
gates (\cref{rem:rs-grid}), each of which is supplied by a distilled
magic state rather than by the code
itself~\cite{bravyi2005magic,litinski2019magic}, so repairing the layer
runs to order
$10^{3}$ $T$ gates.  Thirty-one bytes against a thousand magic states
is the exchange rate in miniature; \cref{sec:ft-consequences} measures
where it ends, at a \QREMaterializeSpacetimeRatio$\times$
spacetime-volume ratio between the aggregate and materialized paths
under a common error budget.

The same picture shows where the statement stops, and it is worth
saying early because a figure is easier to over-read than a theorem.
...
and normalize away (\cref{lem:clifford-interleaving}).  The boundaries
...
estimates~\cite{beverland2022assessing}. What is missing is a
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item A refinement showing that the two payment channels are not
    interchangeable at par.  When a committed record is executed rather
    than buffered, the committed channel additionally pays
    $\log_2\binom{m}{k}$ bits---the entropy of the compiler's own
    decision about which $k$ coordinates to commit early---while the
    state channel pays none, because the stream that follows discloses
    that decision for free.  The toll vanishes exactly when the
    commitment schedule is public, is independent of $m$, and is
    measured on an instrumented device under a preregistration frozen
    before the run (\cref{prop:commitment-toll,fig:commit-toll}).
    ...
    memory-capped hybrid, and materialized-IR reference compilers.  A
    fixed-total-error synthesis/QRE campaign reports logical $T$, data and
    factory qubits, cycles, factories, runtime, and spacetime volume under two
...
\input{../theory/model_implementation_mapping}
...
The next lemma removes the alphabet size from the designer's hands: up to
...
\begin{remark}[Ross--Selinger realization of the grid]
\label{rem:rs-grid}
The synthesis layer realizes exactly this resolution.  Ancilla-free
...
\cite[Prop.~8.2]{ross2016optimal}.  Every grid angle therefore costs the
same $\Theta(\log(1/\epsilon))$ synthesis budget, uniformly over the
alphabet---the uniformity used by the fixed-total-error campaign of
\cref{sec:ft-consequences}, whose rotations are synthesized by the staq
\texttt{grid\_synth} implementation of this algorithm.  Consequently the
lower-bound term $m\log_2K_\epsilon$ is $m\log_2\Theta(1/\epsilon)$ with
the constant pinned by \eqref{eq:grid-calibration-K}, not selected; both
directions of the calibration are stated as two-sided bounds and no
tighter constant is claimed.  For the cyclic grid used by the dispersed
...
Each of the following mechanisms emits scheduled streams of
\cref{def:scheduled-stream}, and each generated stream family satisfies
the hypotheses of \cref{def:w1-dispersed-stream}: every aggregate
$x\in[K]_0^m$, $K=Q-1$, is realized by some valid stream, and no token
...
\begin{proof}
In (a) and (b) the round coefficients are unconstrained across rounds,
...
reach the compiler in dispersed rather than pre-aggregated form and the
compiler state that survives the cut trade along one line.  We call
...
What $\eta$ measures is how much of the phase content upstream chose to
present in late-aggregated form, which is exactly the axis of the
design rule.
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\linewidth]{figures/eta_sweep_pareto.pdf}
...
the zero-error floor $\eta m(\log_23+1)/8$ bytes (dashed; ratios
...
$2.6$ bits---the floor $\log_23+1=2.585$ of residue plus sign---while
...
proposition isolates the part of that gap which is not an artifact of
the serializer.

\begin{proposition}[Commitment toll]
\label{prop:commitment-toll}
Fix $Q$, $K=Q-1$, $\epsilon<\sin(\pi/(2Q))$, and a dispersal mask
$T\subseteq[m]$ with $|T|=k$.  Let $\mathcal A$ be correct on every
valid stream of \cref{def:scheduled-stream} and require its committed
prefix to be independently parseable---the append-only first-exposure
regime of \cref{rem:streaming-grounding}, in which an emitted record is
executed and cannot be reinterpreted once the suffix arrives.  Then at
the inter-round cut
\begin{equation}\label{eq:commitment-toll}
 A_{\mathrm{pre}}\;\geq\;\underbrace{k\log_2(2Q)}_{\text{payload}}
 \;+\;\underbrace{\log_2\binom{m}{k}}_{\text{toll}},
\end{equation}
while a deferred compiler attains $A_{\mathrm{pre}}=0$ with
$S\leq k\log_2(2Q)+O(\log m)$.  The toll is the entropy of the
commitment schedule; it vanishes iff that schedule is determined by
public data, in particular at $k=m$.
\end{proposition}

\begin{proof}
Two valid streams whose round-one content differs in the mask, in a
residue, or in a sign require different round-one rotations, and by
\cref{thm:w1-exact-separation} their completions are separated by
$2\sin(\pi/(2Q))>2\epsilon$; a prefix that is executed as emitted must
therefore differ between them.  The map from
$(T,\{a_j^{(1)}\}_{j\in T},\{s_j^{(1)}\}_{j\in T})$ to committed
prefixes is thus injective on $\binom{m}{k}Q^k2^k$ values, and
prefix-freeness of the declared serializer with Kraft's inequality
gives \eqref{eq:commitment-toll}.  For the deferred compiler, the
round-major order of \cref{def:scheduled-stream} is public, so stored
residues are addressed by position and the mask is disclosed by the
round-two records at no charge; the $O(\log m)$ term is the header.
\end{proof}

The two channels are therefore \emph{not} at par once commitment is
irreversible: committing early costs the entropy of one's own decision
of when to commit, and deferring is handed that same information by the
stream that follows.  Because $\log_2\binom{m}{k}/k\to h_2(\eta)/\eta$,
the toll is bounded by a constant---about $2$ bits per generator at
$\eta=\tfrac12$ and $0$ at $\eta=1$---and does not grow with $m$.

\Cref{fig:commit-toll} tests this on the same device under a
...
\cref{fig:eta-sweep}, an identity-optimal self-identifying prefix, and
a control whose prefix omits the mask and recovers it from the suffix.
...
$16\times$ range of $m$); it is exactly zero at $\eta=1$; and it is
exactly zero at every $\eta$ in the control, confirming that
irreversibility of commitment, not addressing, is what buys the
asymmetry.

This also settles what the $6{:}1$ of \cref{fig:eta-sweep} is.  Under
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\linewidth]{figures/commit_toll.pdf}
...
bits per committed generator paid by a self-identifying committed
prefix in excess of the deferred snapshot, against the predicted
schedule entropy $\log_2\binom{m}{k}/k$ (dashed).  The toll is flat in
$m$ and vanishes at $\eta=1$.  Right: the per-generator exchange rate
...
the commitment schedule is public.  Dotted: the same rate as actually
...
and seed, then crosses every compiler with nine representations:
...
$\Omega(m\log K_\epsilon)$ floor requires, and in fact more---while the
...
\end{split}
\end{equation*}
The authors factorial contributes 20,736 cells and the matched strongest
available external semantic baseline, TKET PauliSimp, contributes 324, for
...
predicts, and \cref{sec:cut-tracing} measures, the state they must carry
to do it.

\paragraph{Gate synthesis and $T$-count accounting.}
The exchange rate in our statement is set by the synthesis grid.
...
approximation algorithms of Ross--Selinger
type~\cite{ross2016optimal,selinger2015efficient} all realize a
$Z$-rotation as a word of length $\Theta(\log(1/\epsilon))$ whose letters
individually carry no part of the angle; multi-qubit $T$-count lower
bounds sharpen the same accounting at the unitary
level~\cite{gheorghiu2022tcount}.  We use these results as the
quantitative source of $K_\epsilon$ and of the repair cost per
mis-committed coordinate, not as objects of study.
...
our model the \textsc{swap} and routing records they insert are Clifford
and therefore interior to a block, so they normalize away
(\cref{lem:clifford-interleaving}) without weakening the bound; the
...
them~\cite{beverland2022assessing,watkins2024surfacecode} make the same
point we rely on in \cref{sec:ft-consequences}: constant-factor
differences upstream of synthesis become order-of-magnitude differences
in spacetime volume, and the margin available to a quadratic speedup is
...
randomized relative-output bound is matched up to framing.  The semantic
  ...
  version.  The traced reference-compiler suite measures the named commitment,
  state, and IR channels directly; the matched and external campaigns show when
  global semantic tools recover compact structure; and the natural-workload
  factorial records positive, null, and negative-control outcomes without
  selection.  Fixed-total-error synthesis/QRE exposes a downstream physical
  consequence under an explicit model.  Every W3--W9 headline measurement is
  generated from the normalized frozen-data registry, while retained legacy
  diagnostics are labelled historical.  Recoverability should be treated
  as a controlled variable in compilation and resource estimation.
...
only through the stated symbolic or dense certificate; \QRECells\
...
and for the recommendation that led to joining Prof.~Li's group.
...
with their care, both for this work and for its author.
...
China under Grant Nos.~62476209 and 62576260, the Key Research and
Development Program of Shaanxi under Grant No.~2024CY2-GJHX-18, and the
Fundamental Research Funds for the Central Universities under Grant
No.~QTZX26097.  X.-H.D. acknowledges
support from the Science, Technology and Innovation Commission of Shenzhen
...
AI assistants were used for drafting and revision, repository and code
assistance, literature discovery, and consistency checks.  Their output was
not treated as a scientific source or as independent proof verification.
The authors retain full responsibility for the manuscript, proofs, code,
data, figures, citations, and conclusions.

This paper takes some trouble over provenance: every printed number is
traceable to a unique source row, every experiment to a frozen registry,
every prediction to a criterion written down before the run.  That being
so---\emph{what one begins with, one must end with} (\emph{Zuo Zhuan},
Xuan 12)---recording the provenance of the framing as well is only seeing
the matter through, even though that provenance does not lie in the physics
literature.

Two passages from the classical Chinese canon shaped parts of this work.
Their bearing is plain and their neighborhood fitting, and so they are
quoted here.
...
know, having borrowed it.  The suggestion taken from the passage is that
...
separates planning early, which it recommends, from grasping early, of
which it warns that ``one who grasps it loses it''; that separation is,
...
and because a framing, like a datum, may rightly be asked by a reader where
it came from.  Translations are J.Y.'s own.

This is the first paper of J.Y.'s academic life.  In writing it, J.Y. has
studied closely and thought deeply, examined the literature broadly,
gathered the words of many and set them down as an account, condensed the
text to bring out its sense, and set forth its purport as fully as
possible---in the hope that it may be of some use in time to come.
\end{acknowledgments}
...
\input{appendix/appendix_C_prxq}
